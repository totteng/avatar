...
uploadBtn.addEventListener('click', () => {
  ...
  if (files.length > 0) {
    ...
    for (let file of files) {
      ...
      reader.onload = (e) => {
        uploadedFileContents.push(e.target.result);
        readCount++;
        if (readCount === files.length) {
          updateStatus(statusElement, 'All files uploaded successfully.');
        }
      };
...
queryBtn.addEventListener('click', async () => {
  ...
  updateStatus(statusElement, 'Querying documents... please wait');

  try {
    const combinedDocuments = uploadedFileContents.join('\n');
    const combinedPrompt = `Documents: ${combinedDocuments}\n\nQuery: ${query}\n\nAnswer:`;
    ...
    updateStatus(statusElement, 'Error querying documents.');
  }
});

function startVoiceRecognition(inputElement, talkButton) {
...
voiceQueryBtn.addEventListener('click', () => {
  startVoiceRecognition(queryInput, queryBtn);
});

voiceTaskBtn.addEventListener('click', () => {
  startVoiceRecognition(taskInput, talkBtn);
});
