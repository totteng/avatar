...
queryBtn.addEventListener('click', async () => {
    ...
    try {
        const response = await fetch('http://localhost:3000/openai/query', {
            ...
            body: JSON.stringify({ document: uploadedFileContent, query }),
        });

        if (response.status === 500) {
            console.error('Server error');
            updateStatus(statusElement, 'Server Error. Please make sure the OpenAI API is set up correctly.');
            throw new Error('Server error');
        } else {
            const data = await response.json();
            queryResult.innerHTML = `<p>Query Result: ${data.result}</p>`;
